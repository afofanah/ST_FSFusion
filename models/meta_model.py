"""
MCD-CKT unified model file
===========================
Sources:   doc 9 (MetaCrossDomainFusion) + doc 10 (MetaCross_DomainFusion)
Additions: all missing features from ST-FSFusion paper (docs 6 & 7)

Bug fixes
---------
MetaTransformer.forward   — renamed local F → D to stop shadowing F = nn.functional
MetaCrossDomainFusion     — CrossDomainFusion now applied per-node (B*N reshape) so
                            the node dimension is preserved; gate reduced over feature
                            dim only; temporal encoder output_dim set to trans_hidden_dim
MetaCross_DomainFusion    — same per-node reshape fix; RobustTransformer output_dim
                            overridden to trans_hidden_dim via task_args
"""
...
def ensure_all_tensors_on_same_device(model, device=None):
    if device is None:
        ...
        for param in model.parameters():
            devices[param.device] = devices.get(param.device, 0) + 1
...
def process_input_data(data, device):
    ...
    if isinstance(data, Data):         return data.to(device)
...
# ── Spatial metrics (§4.5, Eq. 17-18) ───────────────────────────────────────

def compute_spatial_metrics(flow: torch.Tensor, adjacency: torch.Tensor,
                             sensor_positions: Optional[torch.Tensor] = None
                             ) -> Dict[str, float]:
    device = flow.device
    N = adjacency.shape[0]
    adj = adjacency.to(device).float()
    deg  = adj.sum(dim=1).clamp(min=1e-8)
    d    = deg.pow(-0.5)
    D_   = torch.diag(d)
    L    = torch.eye(N, device=device) - D_ @ adj @ D_
    f = flow[-1] if flow.dim() == 2 else flow
    f = f.float()
    Lf     = L @ f
    denom  = (f * f).sum().clamp(min=1e-8)
    dirich = (Lf * Lf).sum() / denom
    rho    = float(1.0 - dirich.item())
    results = {"graph_smoothness": rho, "dirichlet_energy": float(dirich.item())}
    if sensor_positions is not None and flow.dim() == 2:
        pos = sensor_positions.float()
        dists = torch.cdist(pos, pos)
        nearby = (dists < 1.0) & (dists > 0)
        corrs  = []
        for i in range(N):
            js = nearby[i].nonzero(as_tuple=True)[0]
            if len(js) == 0: continue
            fi = flow[:, i] - flow[:, i].mean()
            for j in js:
                fj = flow[:, j] - flow[:, j].mean()
                n1, n2 = fi.norm(), fj.norm()
                if n1 > 1e-8 and n2 > 1e-8:
                    corrs.append(float((fi * fj).sum() / (n1 * n2)))
        results["spatial_correlation_mean"] = float(np.mean(corrs)) if corrs else 0.0
        results["spatial_correlation_std"]  = float(np.std(corrs))  if corrs else 0.0
    return results
...
class STMetaLearner(nn.Module):
    def __init__(self, model_args: Dict, task_args: Dict, device="cuda"):
        super().__init__()
        ...
        orig_dim        = model_args.get("message_dim", self.node_feature_dim)
        self.message_dim= max(orig_dim, 16) // self.num_heads * self.num_heads
    ...
    def _build(self):
        ...
        if self.sp:
            self.sp_learner = GCNConv(
                ...
                out_channels=self.hidden_dim, improved=True,
                add_self_loops=True).to(self.device)
    ...
    @staticmethod
    def nuclear_norm_loss(meta_knowledge: torch.Tensor) -> torch.Tensor:
        losses = []
        for b in range(meta_knowledge.shape[0]):
            losses.append(torch.linalg.matrix_norm(meta_knowledge[b], ord="nuc"))
    ...
    def forward(self, data, dim=3):
        ...
        meta = self.feature_projector(c)
        meta = F.layer_norm(meta, [meta.size(-1)])
        return meta.view(B, N, self.meta_out)

    def _prep3(self, x, B, N):
        D = x.size(2)
        if D != self.message_dim:
            if D > self.message_dim: x = x[:, :, :self.message_dim]
            else: x = torch.cat([x, torch.zeros(B, N, self.message_dim - D,
    ...
    def _prep4(self, x, B, N):
        ...
        if D != self.message_dim:
            if D > self.message_dim: x = x[:, :, :, :self.message_dim]
            else: x = torch.cat([x, torch.zeros(B, N, T, self.message_dim - D,
...
class MetaTransformer(nn.Module):
    ...
    def forward(self, meta_knowledge, data, input=None):
        ...
        # FIX: renamed F → D to stop shadowing torch.nn.functional.F
        if inp.dim() == 4:   B, N, T, D = inp.shape
        elif inp.dim() == 3: B, N, D = inp.shape; T = 1; inp = inp.unsqueeze(2)
        else: raise ValueError(f"Invalid: {inp.shape}")
        flat = inp.reshape(-1, T, D)
        if D != self.input_dim:
            if D < self.input_dim:
                flat = torch.cat([flat, torch.zeros(*flat.shape[:-1], self.input_dim - D,
        ...
        q, k, v = self.q_proj(mf).unsqueeze(1), self.k_proj(mf).unsqueeze(1), self.v_proj(mf).unsqueeze(1)
...
class RobustTransformer(nn.Module):
    def __init__(self, model_args: Dict, task_args: Dict, device="cuda"):
        ...
        self.device      = device if torch.cuda.is_available() else "cpu"
        self.noise_tau   = model_args.get("noise_tau",   0.1)
        self.noise_alpha = model_args.get("noise_alpha", 0.05)
        self.enable_noise= model_args.get("enable_noise", True)
        self.prec_tau    = model_args.get("prec_tau", 1.0)
        self.register_buffer("ema_graph_norm", torch.tensor(1.0))
        self.ema_decay   = 0.99
        self._build(); self.to(self.device)
    ...
    def _build(self):
        ...
        enc = nn.TransformerEncoderLayer(
            ...
            activation=self.activation, batch_first=self.batch_first, device=self.device)
        self.transformer_encoder = nn.TransformerEncoder(
            enc, num_layers=1,
    ...
    def graph_norm(self, x: torch.Tensor) -> float:
        if x.numel() < 2: return 0.0
        flat = x.reshape(-1).float()
        grad_proxy = (flat[1:] - flat[:-1]).pow(2).mean()
        denom      = flat.pow(2).mean().clamp(min=1e-8)
        return float((grad_proxy / denom).item())

    def _inject_input_noise(self, x: torch.Tensor, graph_norm_val: float) -> torch.Tensor:
        if not self.training or not self.enable_noise: return x
        sigma = (self.noise_tau / (1.0 + graph_norm_val)) ** 0.5
        return x + torch.randn_like(x) * sigma

    def _register_grad_noise(self):
        if not self.training or not self.enable_noise: return
        for param in self.parameters():
            if param.requires_grad and param.grad is not None:
                var_proxy = param.grad.var().item() if param.grad.numel() > 1 else 1.0
                scale = self.noise_alpha / (1.0 + var_proxy)
                param.grad += torch.distributions.Laplace(0, scale).sample(param.grad.shape).to(param.device)

    def _use_fp16(self, graph_norm_val: float) -> bool:
        if not torch.cuda.is_available(): return False
        return graph_norm_val < self.prec_tau * self.ema_graph_norm.item()
    ...
    def forward(self, data, A_wave=None):
        ...
        if x.dim() == 4:
            ...
            B = N = None; T, F = x.shape[1], x.shape[2]
        else:
            raise ValueError(f"Expected 3D or 4D, got {x.dim()}D")
        gn_val = self.graph_norm(x)
        if self.training:
            self.ema_graph_norm.mul_(self.ema_decay).add_(gn_val * (1 - self.ema_decay))
        x = self._inject_input_noise(x, gn_val)
        if F != self.input_dim:
            if F < self.input_dim:
                x = torch.cat([x, torch.zeros(*x.shape[:-1], self.input_dim - F,
                                               device=self.device, dtype=x.dtype)], -1)
            else: x = x[..., :self.input_dim]
        use_fp16 = self._use_fp16(gn_val)
...
class CrossDomainFusion(nn.Module):
    def __init__(self, spatial_dim: int, temporal_dim: int,
        ...
        self.spatial_proj = nn.Sequential(
            nn.Linear(spatial_dim,  self.shared_dim),
            nn.LayerNorm(self.shared_dim), nn.ReLU()).to(self.device)
        self.temporal_proj = nn.Sequential(
            nn.Linear(temporal_dim, self.shared_dim),
            nn.LayerNorm(self.shared_dim), nn.ReLU()).to(self.device)
        self.fusion_attention = nn.MultiheadAttention(
            embed_dim=self.shared_dim, num_heads=1, batch_first=True, device=self.device)
        self.fusion_proj = nn.Sequential(
            nn.Linear(self.shared_dim * 2, fusion_dim),
            nn.LayerNorm(fusion_dim)).to(self.device)
        self.register_buffer("ema_rho", torch.tensor(1.0))
        self.ema_decay = 0.99
        self._init_weights(); self.to(self.device)
    ...
    def _criticality(self, sp: torch.Tensor, tp: torch.Tensor,
                     alpha: float = 1.0, beta: float = 0.5) -> float:
        sp_flat  = sp.float().reshape(-1)
        tp_flat  = tp.float().reshape(-1)
        sp_proxy = (sp_flat[1:] - sp_flat[:-1]).pow(2).mean() / sp_flat.pow(2).mean().clamp(1e-8)
        tp_norm  = tp_flat.norm() / (tp_flat.numel() ** 0.5)
        return float(alpha * sp_proxy.item() + beta * tp_norm.item())
    ...
    def forward(self, sp: torch.Tensor, tp: torch.Tensor) -> torch.Tensor:
        ...
        rho = self._criticality(sp, tp)
        if self.training:
            self.ema_rho.mul_(self.ema_decay).add_(rho * (1 - self.ema_decay))
        use_fp16 = torch.cuda.is_available() and (rho < self.ema_rho.item())
        with torch.amp.autocast("cuda", enabled=use_fp16):
...
class TemporalEncoderWrapper(nn.Module):
    def __init__(self, model_args: Dict, task_args: Dict, device="cuda"):
        ...
        if model_args.get("use_meta_transformer", False):
            ...
            self.encoder   = RobustTransformer(model_args, task_args, device=self.device)
            self.use_meta  = False
...
class RandomTransformer(nn.Module):
    ...
    def _build(self):
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            enc, num_layers=self.model_args.get("num_layers", 1),
            norm=nn.LayerNorm(self.hidden_dim) if use_ln else None).to(self.device)
        self.output_proj = nn.Linear(self.hidden_dim, self.output_dim).to(self.device)
        for m in self.modules():
            if isinstance(m, nn.Linear):
    ...
    def forward(self, data, A_wave=None):
        x = (data.x if hasattr(data, "x") else data).to(self.device)
        if x.dim() == 4:   B, N, T, F = x.shape; x = x.view(B * N, T, F)
        elif x.dim() == 3: B = N = None; F = x.shape[2]
        else: raise ValueError(f"Expected 3D/4D, got {x.dim()}D")
        if F != self.input_dim:
            if F < self.input_dim:
                x = torch.cat([x, torch.zeros(*x.shape[:-1], self.input_dim - F,
                                               device=self.device, dtype=x.dtype)], -1)
        ...
        with torch.amp.autocast("cuda", enabled=torch.cuda.is_available()):
            out = self.output_proj(self.transformer_encoder(self.input_proj(x)))
        if B is not None:
            out = out.view(B, N, -1, out.shape[-1])
            if not self.model_args.get("return_sequences", False): out = out[:, :, -1, :]
        return out
...
class MetaCrossDomainFusion(nn.Module):
    ...
    def _build_model(self):
        self.mk_learner = STMetaLearner(self.model_args, self.task_args, device=str(self.device))
        ...
        # FIX: output_dim = trans_hidden_dim so tp_feat last dim == hidden_dim
        self.temporal_encoder = TemporalEncoderWrapper(
            model_args={"meta_dim": self.meta_dim, "message_dim": self.message_dim,
                        "hidden_dim": self.trans_hidden_dim,
                        "num_heads": min(4, max(1, self.trans_hidden_dim // 16)),
                        "num_layers": 2, "dropout": 0.1, "use_meta_transformer": True,
                        "output_dim": self.trans_hidden_dim},
            task_args=self.task_args, device=str(self.device))
        self.cross_domain_fusion = CrossDomainFusion(
            self.gnn_hidden_dim, self.trans_hidden_dim, self.hidden_dim, device=str(self.device))
    ...
    def _build_heads(self):
        ...
        self.uncertainty_predictor = nn.Sequential(
            nn.Linear(self.hidden_dim, self.pred_num, device=self.device), nn.Softplus())
    ...
    def forward(self, data, A_hat=None):
        if isinstance(data, torch.Tensor): x, ei = data.to(self.device), None
        elif hasattr(data, "x"):
            x  = data.x.to(self.device)
            ei = getattr(data, "edge_index", None)
            if ei is not None: ei = ei.to(self.device)
        ...
        if ei is not None:
            ...
            valid = (ei[0] <= N - 1) & (ei[1] <= N - 1); ei_v = ei[:, valid]
            pieces = []
            for b in range(B):
                h = si[b]
                ...
                pieces.append(h)
            sp_feat = torch.stack(pieces)              # (B, N, gnn_hidden_dim)

        tp_feat = self.temporal_encoder(input_data.to(self.device), meta)
        if tp_feat.dim() == 4: tp_feat = tp_feat[:, :, -1, :]   # (B, N, trans_hidden_dim)

        if sp_feat is not None:
            # FIX: apply CrossDomainFusion per-node.
            # The original code did sp_feat.reshape(B,-1) inside CrossDomainFusion,
            # collapsing (B,N,features) → (B, N*features) then truncating to
            # (B, spatial_dim). This destroyed the node dimension, giving a
            # (B, hidden_dim) fused tensor. Multiplying by gate (B,1,1) then
            # broadcast-expanded it to (B, B, hidden_dim) when B>1, making
            # flow_predictor output (B, pred_num, B) instead of (B, pred_num, N).
            sp_2d    = sp_feat.reshape(B * N, self.gnn_hidden_dim)    # (B*N, gnn_hidden_dim)
            tp_2d    = tp_feat.reshape(B * N, self.trans_hidden_dim)  # (B*N, trans_hidden_dim)
            fused_2d = self.cross_domain_fusion(sp_2d, tp_2d)         # (B*N, hidden_dim)
            fused    = fused_2d.reshape(B, N, self.hidden_dim)        # (B, N, hidden_dim)
            # FIX: gate over feature dim only to get (B, N, 1), not (B, 1, 1)
            gate  = torch.sigmoid(meta.mean(dim=2, keepdim=True)).to(self.device)
            fused = fused * gate
        else:
            fused = tp_feat   # (B, N, trans_hidden_dim) == (B, N, hidden_dim)

        # (B, N, hidden_dim) → (B, N, pred_num) → (B, pred_num, N)
...
# ── MetaCross_DomainFusion ───────────────────────────────────────────────────

class MetaCross_DomainFusion(nn.Module):
    """
    Lightweight variant (doc 10).  gnn_hidden=64, trans_hidden=32.
    Output: flow_pred (B,pred_num,N), uncertainty (B,pred_num,N), graph (N,N)
    """
    def __init__(self, model_args: Dict, task_args: Dict, device="cuda"):
        super().__init__()
    ...
    def _build_model(self):
        self.mk_learner = STMetaLearner(self.model_args, self.task_args, device=self.device)
        self.spatial_encoder = nn.ModuleList([
            GCNConv(in_channels=self.message_dim * self.his_num,
                    out_channels=self.gnn_hidden_dim),
            nn.LayerNorm(self.gnn_hidden_dim), nn.ReLU()])
        # FIX: override output_dim in task_args so RobustTransformer outputs
        #      trans_hidden_dim instead of pred_num
        _task_args_enc = {**self.task_args, "output_dim": self.trans_hidden_dim}
        self.temporal_encoder = TemporalEncoderWrapper(
            model_args={"meta_dim": self.meta_dim, "message_dim": self.message_dim,
                        "hidden_dim": self.trans_hidden_dim,
                        "num_heads": min(2, max(1, self.trans_hidden_dim // 16)),
                        "num_layers": 1, "dropout": 0.1, "use_meta_transformer": False,
                        "enable_noise": True},
            task_args=_task_args_enc, device=self.device)
        self.cross_domain_fusion = CrossDomainFusion(
            self.gnn_hidden_dim, self.trans_hidden_dim, self.hidden_dim, device=self.device)
        self.meta_attention = nn.Sequential(
            nn.Linear(self.meta_dim, self.hidden_dim, device=self.device), nn.Sigmoid())
    ...
    def forward(self, data, A_hat=None):
        ...
        meta  = self.mk_learner(data, dim=3)           # (B, N, meta_dim)
        graph = torch.eye(N, device=self.device)
        sp_feat = None
        if ei is not None:
            si = input_data.reshape(B, N, -1)
            ...
            for b in range(B):
                ...
                pieces.append(h)
            sp_feat = torch.stack(pieces)              # (B, N, gnn_hidden_dim)
        tp_feat = self.temporal_encoder(input_data)    # (B, N, trans_hidden_dim)
        if tp_feat.dim() == 4: tp_feat = tp_feat[:, :, -1, :]

        if sp_feat is not None:
            # FIX: per-node CrossDomainFusion — same rationale as MetaCrossDomainFusion
            sp_2d    = sp_feat.reshape(B * N, self.gnn_hidden_dim)
            tp_2d    = tp_feat.reshape(B * N, self.trans_hidden_dim)
            fused_2d = self.cross_domain_fusion(sp_2d, tp_2d)         # (B*N, hidden_dim)
            fused    = fused_2d.reshape(B, N, self.hidden_dim)        # (B, N, hidden_dim)
            gate  = torch.sigmoid(meta.mean(dim=2, keepdim=True))     # (B, N, 1)
            fused = fused * gate
        else:
            fused = tp_feat

        with torch.amp.autocast("cuda", enabled=torch.cuda.is_available()):
            flow = self.flow_predictor(fused)
            unc  = self.uncertainty_predictor(fused)
        return flow.transpose(1, 2).contiguous(), unc.transpose(1, 2).contiguous(), graph
...
class TransformerBasedPredictor(nn.Module):
    ...
    def forward(self, data, A_wave=None):
        ...
        unc   = self.unc_head(fused).transpose(1, 2).contiguous() if hasattr(self, "unc_head") else None
        return preds, unc, None
...
class CrossCityAdapter(nn.Module):
    def __init__(self, source_dim: int, target_dim: int,
                 hidden_dim: int = 128, device="cuda"):
        super().__init__()
    ...
    def forward(self, source: torch.Tensor, target: torch.Tensor):
        ...
        for t in (source, target):
            if not isinstance(t, torch.Tensor): raise TypeError("Inputs must be Tensor")
        ...
        B = source.size(0)
        if B * source.size(1) * target.size(1) * self.hidden_dim > 1e9 and dev.type in ("mps","cuda"):
            return self._cpu_fallback(source, target)
    ...
    def _cpu_fallback(self, source, target):
        orig = next(self.parameters()).device; self.cpu()
        cs, ct = source.cpu(), target.cpu()
        sa = self.source_adapter(cs); ta = self.target_adapter(ct)
        chunk = min(1000, ct.size(1)); aws, trs = [], []
        for i in range(0, ct.size(1), chunk):
            tc = ta[:, i:i+chunk]; sim = torch.bmm(tc, sa.transpose(1,2)) / self.hidden_dim ** 0.5
            aw = F.softmax(sim, -1); aws.append(aw); trs.append(torch.bmm(aw, sa))
        w, tr = torch.cat(aws, 1), torch.cat(trs, 1)
        g = torch.sigmoid(ta.mean(-1, keepdim=True))
        enh = ta * (1 - g) + tr * g
        if self.output_proj is not None: enh = self.output_proj(enh)
        self.to(orig)
        return enh.to(orig), w.to(orig)

    def transfer_meta_knowledge(self, source_meta: torch.Tensor, target_meta: torch.Tensor):
        dev = next(self.parameters()).device
...
class FewShotTrafficLearner:
    ...
    def _discretize(self, values: torch.Tensor, num_bins: int = 10):
        flat = values.reshape(-1); lo, hi = flat.min(), flat.max()
        b = torch.floor((values - lo) / ((hi - lo) / num_bins + 1e-8)).long()
    ...
    def _intermediate(self, data):
        ...
        if tp_feat.dim() == 4: tp_feat = tp_feat[:, :, -1, :]
        if sp_feat is not None:
            sp_2d    = sp_feat.reshape(B * N, self.base_model.gnn_hidden_dim)
            tp_2d    = tp_feat.reshape(B * N, self.base_model.trans_hidden_dim)
            fused_2d = self.base_model.cross_domain_fusion(sp_2d, tp_2d)
            fused    = fused_2d.reshape(B, N, self.base_model.hidden_dim)
            fused    = fused * torch.sigmoid(meta.mean(dim=2, keepdim=True))
        else: fused = tp_feat
        return fused

    @staticmethod
    def graph_recon_loss(embeddings: torch.Tensor, adjacency: torch.Tensor,
                         sparse: bool = True) -> torch.Tensor:
        Z     = embeddings.float()
        A     = adjacency.to(Z.device).float()
        A_hat = torch.sigmoid(torch.bmm(Z, Z.transpose(1, 2)) if Z.dim() == 3
                              else Z @ Z.t())
        if Z.dim() == 2: A_hat = A_hat.unsqueeze(0); A = A.unsqueeze(0)
        if sparse:
            eps = 1e-7
            return F.binary_cross_entropy(A_hat.clamp(eps, 1 - eps), A.clamp(0, 1))
        else:
            return F.mse_loss(A_hat, A)

    def hybrid_loss(self, predictions: torch.Tensor, targets: torch.Tensor,
                    embeddings: torch.Tensor, binned_labels: torch.Tensor,
                    prototypes: Dict, adjacency: Optional[torch.Tensor] = None
                    ) -> Tuple[torch.Tensor, Dict]:
        regression_loss = F.mse_loss(predictions.float(), targets.float())
        proto_loss = torch.tensor(0.0, device=predictions.device)
        Z_flat     = embeddings.reshape(-1, embeddings.shape[-1])
        b_flat     = binned_labels.reshape(-1)
        pf         = self.proto_encoder(Z_flat)
        for b_id, proto in prototypes.items():
            mask = (b_flat == b_id)
            if mask.any():
                proto_loss = proto_loss + (pf[mask] - proto.unsqueeze(0)).pow(2).sum(-1).mean()
        gr_loss = torch.tensor(0.0, device=predictions.device)
        if adjacency is not None and embeddings.dim() >= 2:
            Z3 = embeddings if embeddings.dim() == 3 else embeddings.unsqueeze(0)
            A3 = adjacency if adjacency.dim() == 3 else adjacency.unsqueeze(0)
            gr_loss = self.graph_recon_loss(Z3, A3, sparse=(adjacency.mean() < 0.1))
        total = regression_loss + self.lambda1 * proto_loss + self.lambda2 * gr_loss
        return total, {"regression": regression_loss.item(),
                       "proto_align": proto_loss.item(),
                       "graph_recon": gr_loss.item()}
    ...
    def create_support_prototypes(self, support_data, support_labels):
        with torch.no_grad():
            ...
            return {b.item(): pf[bins == b].mean(0) for b in torch.unique(bins) if (bins == b).any()}

    def adapt_to_target(self, support_data, support_labels,
                        adjacency: Optional[torch.Tensor] = None,
                        num_steps: int = 10, lr: float = 0.001,
                        epsilon: float = 0.1) -> "MetaCrossDomainFusion":
        protos   = self.create_support_prototypes(support_data, support_labels)
        adapted  = copy.deepcopy(self.base_model)
        theta_0  = {n: p.clone().detach() for n, p in adapted.named_parameters()}
        opt      = torch.optim.Adam(list(self.adaptation_layers.parameters())
                                    + list(self.proto_encoder.parameters()), lr=lr)
        bin_ids  = sorted(protos.keys()); bv = torch.tensor(bin_ids, device=self.device, dtype=torch.float)
        lo, hi   = support_labels.min(), support_labels.max()
        bw       = (hi - lo) / len(bin_ids)
        for _ in range(num_steps):
            for data, label in zip(support_data, support_labels):
                feats = self._intermediate(data)
                pf    = self.proto_encoder(feats)
                bins  = self._discretize(label)
                lg    = torch.stack(
                    [-((pf - protos[b].unsqueeze(0)).pow(2).sum(-1)) / self.temperature
                     for b in bin_ids], -1)
                pr    = F.softmax(lg, -1)
                pred  = lo + (pr * bv.unsqueeze(0).unsqueeze(0)).sum(-1) * bw + bw / 2
                for layer in self.adaptation_layers: pred = layer(pred)
                total, _ = self.hybrid_loss(pred, label, feats, bins, protos, adjacency)
                opt.zero_grad(); total.backward()
                with torch.no_grad():
                    for name, param in adapted.named_parameters():
                        if name in theta_0:
                            diff  = param.data - theta_0[name]
                            norm  = diff.norm()
                            if norm > epsilon:
                                param.data = theta_0[name] + diff * (epsilon / norm)
                opt.step()
...
def create_model(model_args: Dict, task_args: Dict, device: str = "cpu") -> nn.Module:
    mtype = model_args.get("type", "MetaCrossDomainFusion")
    if mtype == "MetaCross_DomainFusion": m = MetaCross_DomainFusion(model_args, task_args, device)
    elif mtype == "TransformerBased":     m = TransformerBasedPredictor(model_args, task_args, device)
    elif mtype == "RandomTransformer":    m = RandomTransformer(model_args, task_args, device)
    elif mtype == "RobustTransformer":    m = RobustTransformer(model_args, task_args, device)
...
create_optimized_model = create_model
